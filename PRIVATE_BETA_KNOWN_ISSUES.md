...
| PBAT-012 | High | Database Diagnostics | Fixed, Pending VM Re-Test | Direct backend registration returned `503 service_unavailable` / `Database operation failed`. | Acceptance test reached `POST /auth/register`, but the API response was intentionally generic and backend logs lacked enough structured diagnostic detail. | Fixed by adding server-side exception logging for database operation failures while keeping API responses generic; verify migrations created `users` and re-run registration. |
...
- PBAT-012: Direct auth registration returns 503 and requires database diagnostics/retest.
